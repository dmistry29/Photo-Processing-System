...
public class JUnitTests {
	...
	public void pubRelOne() {
		...
		if (!manager.addToQueue("Park, Larry", "Jaws")) {
			resultsBuffer.append("Failed to add to queue");
		}

		if (!manager.addToQueue("Park, Larry", "Jaws")) {
			resultsBuffer.append("Failed to add to queue as \"Jaws\" is already in\n");
		}

		if (!manager.addToQueue("Park, Larry", "Bad")) {
			resultsBuffer.append("Failed to add to queue");
		}
		...
		if (!manager.addToQueue("Albert, Mike", "Gone with the Wind")) {
			resultsBuffer.append("Failed to add to queue");
		}
	...
	public void pubRelTwo() {
		...
		resultsBuffer.append("After processing\n");
		resultsBuffer.append(manager.processRequests());
		resultsBuffer.append(manager.getAllCustomersInfo());
		resultsBuffer.append(manager.getAllMediaInfo());
	...
	public void pubRelThree() {
		...
		ArrayList<String> searchResults = manager.searchMedia(null, null, null, null);
		resultsBuffer.append("\nNull Search: " + searchResults);
		
		searchResults = manager.searchMedia("Jaws", null, null, null);
		resultsBuffer.append("\nTitle Search: " + searchResults);
		
		searchResults = manager.searchMedia(null, null, null, "day, He");
		resultsBuffer.append("\nSong Search: " + searchResults);
